...
public class Chunkdata implements Serializable {
    ...
    public void seekSpace(int dimention, int minX, int maxX, int minZ, int maxZ) {
        map.seekSpace(dimention, minX, maxX, minZ, maxZ);
    }

    public void seekTime(int gametick) {
        map.seekTime(gametick);
    }

    public int[][][] getRenderColors() {
        return map.renderColors();
    }

    public ChunkView chunkEventData(int x, int z) {
        return map.pickChunk(x, z);
    }
    ...
    public class MapView implements Iterable<ChunkView> {
        ...
        public void clear() {
            chunkViews = new ChunkView[0];
            clearCount = Chunkdata.this.clearCount;
            gametick = 0;
            minx = 0;
            maxx = 0;
            minz = 0;
        ...
        public int[][][] renderColors() {
            int sizeX = maxx - minx;
            int sizeZ = maxz - minz;
            int[][][] list = new int[sizeX][sizeZ][];
            for (int z = 0; z < sizeZ; z++) {
                for (int x = 0; x < sizeX; x++) {
                    ChunkView cv = pickChunk(x + minx, z + minz);
                    if (cv != null) {
                        list[x][z] = cv.getColors();
                    } else {
                        list[x][z][0] = 0;
                    }
                }
