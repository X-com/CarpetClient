...
public class Chunkdata {
	...
	static class ChunkLogChunkCoords {
		int x;
		int z;
		int d;
	...
	static class ChunkLogTimeCoords {
		int gametick;
		int eventNumber;
	...
	static class ChunkLogCoords {
		ChunkLogChunkCoords space;
		ChunkLogTimeCoords time;
		...
		public void seekTime(int gametick) {
			if(!isNextGametick(gametick)) {
				...
				for(int z=minz; z<maxz; ++z) {
					...
					chunkLineNow[0] = playerLogs.logsGroupedByTime.subMap(minChunk, maxChunk);
